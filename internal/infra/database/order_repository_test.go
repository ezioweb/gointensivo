...
import (
	...
	"github.com/ezioweb/gointensivo/internal/entity"
...
func (suite *OrderRepositoryTestSuite) SetupSuite() {
	...
	suite.NoError(err)
	db.Exec("CREATE TABLES orders (id varchar(255) NOT NULL, price float NOT NULL, tax float NOT NULL, final_price float NOT NULL, PRIMARY KEY (id))")
	suite.Db = db
}
...
func (suite *OrderRepositoryTestSuite) TestSavingOrder() {
	order, err := entity.NewOrder("Order01", 10.0, 1.0)
	suite.NoError(err)
